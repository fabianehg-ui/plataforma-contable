...
class MotorClasificacion:
    ...
    def clasificar_movimiento(
        ...
        if capa3:
            ...
            #   - Nueva: excluir=True con motivo_exclusion
            #   - Legacy: formato_dian == '999999' (la usaba la UI antigua)
            def _es_exclusion(r) -> bool:
                if r.excluir:
                    return True
                if (r.formato_dian or '').strip() == '999999':
                    return True
                return False

            regla_excluyente = next((r for r in capa3 if _es_exclusion(r)), None)
            ...
            for r in capa3:
                if not r.formato_dian or _es_exclusion(r):
