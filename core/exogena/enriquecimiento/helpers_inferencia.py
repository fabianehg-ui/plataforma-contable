...
def validar_tercero_completo(
    ...
    """
    errores = []
    es_natural = es_persona_natural(tercero_dict)

    # === Campo identidad mínimo ===
    razon = (tercero_dict.get('razon_social') or '').strip()
    primer_apellido = (tercero_dict.get('primer_apellido') or '').strip()
    primer_nombre = (tercero_dict.get('primer_nombre') or '').strip()

    if es_natural:
        # Persona natural: DEBE tener primer_apellido y primer_nombre.
        # NO debe tener razón social (van separados en columnas).
        if not primer_apellido:
            errores.append("Persona natural sin primer apellido")
        if not primer_nombre:
            errores.append("Persona natural sin primer nombre")
        if razon and not (primer_apellido and primer_nombre):
            # Caso típico: todo el nombre metido en razon_social
            errores.append(
                "Nombre completo en columna 'Razón social' — debe dividirse "
                "en apellidos y nombres"
            )
    else:
        # Persona jurídica: DEBE tener razón social.
        # NO debe tener apellidos/nombres (son personas).
        if not razon:
            errores.append("Persona jurídica sin razón social")
...
def auto_dividir_nombre_natural(tercero_dict: dict) -> dict:
    """
    Si una persona natural tiene todo su nombre en `razon_social` (ej.
    "JORGE IVAN GONZALEZ RESTREPO") y los campos apellido/nombre están
    vacíos, divide automáticamente el nombre en los 4 campos:

      "JORGE IVAN GONZALEZ RESTREPO" →
        primer_apellido = "GONZALEZ"
        segundo_apellido = "RESTREPO"
        primer_nombre = "JORGE"
        otros_nombres = "IVAN"

    Heurística de los apellidos colombianos: las 2 últimas palabras suelen
    ser los apellidos, las primeras los nombres.

    Args:
        tercero_dict: dict del tercero (puede tener razon_social).

    Returns:
        Copia con primer_apellido, segundo_apellido, primer_nombre y
        otros_nombres llenos, y razon_social vaciada (si era natural).
    """
    out = dict(tercero_dict)
    if not es_persona_natural(out):
        return out

    # Si ya tiene apellido y nombre, no tocar nada
    if (out.get('primer_apellido') or '').strip() and (out.get('primer_nombre') or '').strip():
        return out

    razon = (out.get('razon_social') or '').strip()
    if not razon:
        return out

    # Limpiar el nombre: quitar comas, tildes raras, puntuación
    partes = [p for p in razon.replace(',', ' ').replace('.', ' ').split() if p]
    if len(partes) < 2:
        return out  # No podemos dividir un solo nombre

    # Heurística: las últimas 2 palabras son apellidos; las anteriores son nombres
    # Casos comunes:
    #   3 palabras: 1 nombre + 1 apellido + 1 apellido → JORGE GONZALEZ RESTREPO
    #   4 palabras: 2 nombres + 2 apellidos → JORGE IVAN GONZALEZ RESTREPO
    #   5+ palabras: lo restante todo en otros_nombres
    if len(partes) == 2:
        # Solo 2 palabras: asumir nombre + apellido
        out['primer_nombre'] = partes[0]
        out['primer_apellido'] = partes[1]
        out['segundo_apellido'] = None
        out['otros_nombres'] = None
    elif len(partes) == 3:
        # 3 palabras: 1 nombre + 2 apellidos
        out['primer_nombre'] = partes[0]
        out['otros_nombres'] = None
        out['primer_apellido'] = partes[1]
        out['segundo_apellido'] = partes[2]
    elif len(partes) == 4:
        # 4 palabras: 2 nombres + 2 apellidos (lo más común)
        out['primer_nombre'] = partes[0]
        out['otros_nombres'] = partes[1]
        out['primer_apellido'] = partes[2]
        out['segundo_apellido'] = partes[3]
    else:
        # 5+: las últimas 2 son apellidos, la primera es nombre, el resto otros nombres
        out['primer_apellido'] = partes[-2]
        out['segundo_apellido'] = partes[-1]
        out['primer_nombre'] = partes[0]
        out['otros_nombres'] = ' '.join(partes[1:-2])

    # Vaciar razon_social ya que ahora está dividido
    out['razon_social'] = None
    out['_auto_dividido'] = True
