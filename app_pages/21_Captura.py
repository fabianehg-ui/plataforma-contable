...
with st.sidebar:
    ...
    if not comprobantes:
        st.caption("No tienes comprobantes creados aún.")
    else:
        for c in comprobantes:
            st.caption(f"**{c['codigo']}** · {c['nombre']}")
    with st.expander("➕ Crear / actualizar tipo"):
        nc_cod = st.text_input("Código", key="nc_cod")
        nc_nom = st.text_input("Nombre", key="nc_nom")
        if st.button("Guardar tipo", key="btn_nc"):
        ...
        if st.button("Crear los 4 sugeridos", key="btn_sug"):
...
if not comprobantes:
    st.info(
        "Primero crea al menos un **tipo de comprobante** en la barra lateral "
        "(o usa «Crear los 4 sugeridos»: recibo de caja, egreso, causación, nota)."
    )
    st.stop()
